/**
 *
 * @desc code to build HTML user interface for /{PNAME}/group endpoint
...
function _vise_show_group_ui() {
  ...
  var group_icon = _vise_common_get_svg_button('micon_group', 'Show visual groups (i.e. images grouped based on their content)');
  ...
  if(_vise_data.GROUP.hasOwnProperty('STATUS')) {
    ...
    if(_vise_data.GROUP['STATUS'] === 'group_index') {
      var html = '<h3>Group Index</h3><p>' + _vise_data.GROUP['MESSAGE'] + '</p><ul>';
      for(var i=0; i<_vise_data.GROUP['group_id_list'].length; ++i) {
        var group_id = _vise_data.GROUP['group_id_list'][i];
        html += '<li><a href="group?group_id=' + group_id + '">' + group_id + '</a></li>';
      }
      html += '</ul>'
...
function _vise_init_group_toolbar() {
  ...
  start_input.addEventListener('change', function(e) {
    var new_start = parseInt(this.value);
      ...
      var url = [ 'group?group_id=' + _vise_data.GROUP['group_id'] ];
...
function _vise_init_group_filter_toolbar() {
  ...
  group_info_container.innerHTML = 'Show visual groups (i.e. images grouped based on their content) with following properties:&nbsp;';
  ...
  set_size_label.innerHTML = 'Number of images in each set&nbsp;';
...
function _vise_init_group_content() {
  ...
  for(var set_index in _vise_data.GROUP['set_id_list']) {
    ...
    set_query_caption.innerHTML = setdata['query_filename'];
    ...
    for (var i=0; i<setdata['match_filename_list'].length; ++i) {
      ...
      set_match_caption.innerHTML = setdata['match_filename_list'][i];
